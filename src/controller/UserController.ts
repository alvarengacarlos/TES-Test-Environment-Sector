...
export class UserController {
    ...
    async signUp(
        httpRequest: HttpRequest<SignUpDtoInput>,
    ): Promise<HttpResponse<SignUpDtoOutput | null>> {
        try {
            const signUpDtoOutput = await this.userService.signUp(
                httpRequest.data,
            )
            return HttpResponse.created(
                "Sign up executed with success",
                signUpDtoOutput,
            )
        } catch (error: any) {
    ...
    async confirmSignUp(
        httpRequest: HttpRequest<ConfirmSignUpDtoInput>,
    ): Promise<HttpResponse<ConfirmSignUpDtoOutput | null>> {
        try {
            const confirmSignUpDtoOutput = await this.userService.confirmSignUp(
                httpRequest.data,
            )
            return HttpResponse.ok(
                "Confirm sign up executed with success",
                confirmSignUpDtoOutput,
            )
        } catch (error: any) {
    ...
    async signIn(
        httpRequest: HttpRequest<SignInDtoInput>,
    ): Promise<HttpResponse<SignInDtoOutput | null>> {
        try {
            const signInDtoOutput = await this.userService.signIn(
                httpRequest.data,
            )
            return HttpResponse.ok(
                "Sign in executed with success",
                signInDtoOutput,
            )
        } catch (error: any) {
