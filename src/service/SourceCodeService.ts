...
export class SourceCodeService {
    ...
    async uploadSourceCode(
        uploadSourceCodeDtoInput: UploadSourceCodeDtoInput,
    ): Promise<UploadSourceCodeDtoOutput> {
        const findAwsCredentialsDtoOutput =
        ...
        const awsCredentials = new AwsCredentialsEntity(
            findAwsCredentialsDtoOutput.accessKeyId,
            findAwsCredentialsDtoOutput.secretAccessKey,
        )
    ...
    async findSourceCodes(
        findSourceCodesDtoInput: FindSourceCodesDtoInput,
    ): Promise<FindSourceCodesDtoOutput> {
        const findAwsCredentialsDtoOutput =
            await this.awsCredentialsService.findAwsCredentials({
        ...
        const sourceCodeEntities =
            await this.sourceCodeRepository.findSourceCodes({
                awsCredentials: awsCredentials,
            })
    ...
    async deleteSourceCode(
        deleteSourceCodeDtoInput: DeleteSourceCodeDtoInput,
    ): Promise<DeleteSourceCodeDtoOutput> {
        const findAwsCredentialsDtoOutput =
            await this.awsCredentialsService.findAwsCredentials({
