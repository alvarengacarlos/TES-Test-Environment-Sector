...
export class InfraStackService {
    ...
    async createInfraStack(
        createInfraStackDtoInput: CreateInfraStackDtoInput,
    ): Promise<CreateInfraStackDtoOutput> {
        const findAwsCredentialsOutput =
            await this.awsCredentialsService.findAwsCredentials({
                ownerEmail: createInfraStackDtoInput.ownerEmail,
            })
        const awsCredentials = new AwsCredentialsEntity(
            findAwsCredentialsOutput.accessKeyId,
            findAwsCredentialsOutput.secretAccessKey,
        )

        const templateBody = await this.chooseTemplateModel(
            createInfraStackDtoInput.templateType,
        )
    ...
    private async chooseTemplateModel(templateType: string): Promise<string> {
        if (templateType == TemplateModel.CONTAINER_MODEL) {
            const getContainerModelTemplateDtoOutput =
                await this.templateService.getContainerModelTemplate()
    ...
    async findInfraStacks(
        findInfraStacksDtoInput: FindInfraStacksDtoInput,
    ): Promise<FindInfraStacksDtoOutput> {
        const findAwsCredentialsOutput =
            await this.awsCredentialsService.findAwsCredentials({
        ...
        const infraStackEntities =
            await this.infraStackRepository.findInfraStacks({
                awsCredentials: awsCredentials,
            })
    ...
    async deleteInfraStack(
        deleteInfraStackDtoInput: DeleteInfraStackDtoInput,
    ): Promise<DeleteInfraStackDtoOutput> {
        const findAwsCredentialsOutput =
            await this.awsCredentialsService.findAwsCredentials({
...
export class FindInfraStacksDtoOutput {
    constructor(public readonly infraStacks: Array<InfraStackEntity | null>) {}
}
