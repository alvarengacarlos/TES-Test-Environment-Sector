...
export class AwsCredentialsValidator {
    saveAwsCredentialsValidator(
        request: Request,
        response: Response,
        next: NextFunction,
    ) {
        const schema = Joi.object({
            accessKeyId: Joi.string().trim().min(20).max(20).required(),
            secretAccessKey: Joi.string().trim().min(40).max(40).required(),
        })
