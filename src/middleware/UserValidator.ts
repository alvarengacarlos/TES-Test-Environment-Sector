...
export class UserValidator {
    signUpValidator(request: Request, response: Response, next: NextFunction) {
        const schema = Joi.object({
            ...
            password: Joi.string()
                ...
                .pattern(/^(?=.*[a-z]).*$/)
                .message(
                    '"password" must have at least 1 lowercase alphabetical character',
                )
                .pattern(/^(?=.*[A-Z]).*$/)
                .message(
                    '"password" must have at least 1 uppercase alphabetical character',
                )
                .pattern(/^(?=.*\d).*$/)
    ...
    confirmSignUpValidator(
        ...
        const schema = Joi.object({
            email: Joi.string().email().trim().required(),
            confirmationCode: Joi.string().trim().min(6).max(6).required(),
        })
    ...
    signInValidator(request: Request, response: Response, next: NextFunction) {
        const schema = Joi.object({
            email: Joi.string().email().trim().required(),
            password: Joi.string()
                .trim()
