...
export class SourceCodeRepositoryImpl implements SourceCodeRepository {
    ...
    async uploadSourceCode(uploadSourceCodeInput: UploadSourceCodeInput): Promise<void> {
        const s3Client = this.getS3Client(
            uploadSourceCodeInput.awsCredentials.accessKeyId,
            uploadSourceCodeInput.awsCredentials.secretAccessKey
        )

        const sourceCodeStorageExists = await this.sourceCodeStorageExists(s3Client)
        if (!sourceCodeStorageExists) {
            await this.createSourceCodeStorage(s3Client)
        }

        try {
            Logger.info(this.constructor.name, this.uploadSourceCode.name, "executing put object command")
            ...
            Logger.info(this.constructor.name, this.uploadSourceCode.name, "put object command executed with success")

        } catch (error: any) {
            Logger.error(this.constructor.name, this.uploadSourceCode.name, `S3 client throw ${error.message}`)
            throw new S3Exception()
        }
    }

    private async sourceCodeStorageExists(s3Client: S3Client): Promise<boolean> {
        try {
    ...
    async findSourceCodes(findSourceCodesInput: FindSourceCodesInput): Promise<Array<SourceCodeEntity | null>> {
        try {
            Logger.info(this.constructor.name, this.findSourceCodes.name, "executing list objects command")
            const s3Client = this.getS3Client(
            ...
            Logger.error(this.constructor.name, this.findSourceCodes.name, `S3 client throw ${error.message}`)
            throw new S3Exception()
        }
    }

    async deleteSourceCode(deleteSourceCodeInput: DeleteSourceCodeInput): Promise<void> {
        try {
            Logger.info(this.constructor.name, this.deleteSourceCode.name, "executing delete object command")
            const s3Client = this.getS3Client(
            ...
            Logger.info(this.constructor.name, this.deleteSourceCode.name, "delete object command executed with success")

        } catch (error: any) {
            Logger.error(this.constructor.name, this.deleteSourceCode.name, `S3 client throw ${error.message}`)
            throw new S3Exception()
        }
    }
}