...
export class TemplateRepositoryImpl implements TemplateRepository {
    ...
    async getContainerModelTemplate(): Promise<string> {
        try {
            Logger.info(this.constructor.name, this.getContainerModelTemplate.name, `executing get object command`)
            ...
            Logger.info(this.constructor.name, this.getContainerModelTemplate.name, `get object command executed with success`)
            return String(await output.Body?.transformToString())

        } catch (error: any) {
            Logger.error(this.constructor.name, this.getContainerModelTemplate.name, `S3 client throw ${error.message}`)
