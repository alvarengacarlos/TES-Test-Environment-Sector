...
import {
    ...
    SaveUserInput,
    UpdateUserEmailToVerifiedInput,
    UserRepository,
} from "./UserRepository"
    ...
    async saveUser(saveUserInput: SaveUserInput): Promise<UserEntity> {
        try {
            ...
            Logger.info(
                this.constructor.name,
                this.saveUser.name,
                "sign up command executed with success",
            )
            return new UserEntity(saveUserInput.email, saveUserInput.password)
        } catch (error: any) {
            if (error instanceof UsernameExistsException) {
                Logger.warn(
                    this.constructor.name,
                    this.saveUser.name,
                    "sign up command throw username exists",
                )
            ...
            Logger.error(
                this.constructor.name,
                this.saveUser.name,
                `sign up command throw unhandled error: ${error.message}`,
            )
            throw new CognitoException()
        }
    }

    async updateUserEmailToVerified(
        updateUserEmailToVerifiedInput: UpdateUserEmailToVerifiedInput,
    ): Promise<UserEntity> {
        try {
            const confirmSignUpCommand = new ConfirmSignUpCommand({
                ClientId: String(process.env.COGNITO_CLIENT_ID),
                SecretHash: await createCognitoSecretHash(
                    updateUserEmailToVerifiedInput.email,
                ),
                Username: updateUserEmailToVerifiedInput.email,
                ConfirmationCode:
                    updateUserEmailToVerifiedInput.confirmationCode,
            })

            Logger.info(
                this.constructor.name,
                this.updateUserEmailToVerified.name,
                "executing confirm sign up command",
            )
            ...
            Logger.info(
                this.constructor.name,
                this.updateUserEmailToVerified.name,
                "confirm sign up command executed with success",
            )
            return new UserEntity(updateUserEmailToVerifiedInput.email, "")
        } catch (error: any) {
            if (error instanceof ExpiredCodeException) {
                Logger.warn(
                    this.constructor.name,
                    this.updateUserEmailToVerified.name,
                    "confirm sign up command throw expired code",
                )
            ...
            if (error instanceof CodeMismatchException) {
                Logger.warn(
                    this.constructor.name,
                    this.updateUserEmailToVerified.name,
                    "confirm sign up command throw code mismatch",
                )
            ...
            if (error instanceof NotAuthorizedException) {
                Logger.warn(
                    this.constructor.name,
                    this.updateUserEmailToVerified.name,
                    "confirm sign up command throw not authorized",
                )
                throw new InvalidConfirmationCodeException()
            }
            ...
            Logger.error(
                this.constructor.name,
                this.updateUserEmailToVerified.name,
                `confirm sign up command throw unhandled error: ${error.message}`,
            )
            throw new CognitoException()
        }
    }
        ...
        try {
            const adminInitiateAuthCommand = new AdminInitiateAuthCommand({
                ...
                AuthParameters: {
                    ...
                    SECRET_HASH: await createCognitoSecretHash(
                        authenticateUserInput.email,
                    ),
                },
            })
            ...
            if (error instanceof UserNotFoundException) {
                Logger.warn(
                    ...
                    "sign in command throw user not found",
                )
            ...
            if (error instanceof NotAuthorizedException) {
                Logger.warn(
                    this.constructor.name,
                    this.authenticateUser.name,
                    "sign in command throw not authorized",
                )
                throw new IncorrectEmailOrPasswordException()
            }
            ...
            if (error instanceof UserNotConfirmedException) {
                Logger.warn(
                    this.constructor.name,
                    this.authenticateUser.name,
                    "sign in command throw user not confirmed",
                )
            ...
            Logger.error(
                this.constructor.name,
                this.authenticateUser.name,
                `sign in command throw unhandled error: ${error.message}`,
            )
            throw new CognitoException()
        }
    }
