...
export class AwsCredentialsRepositoryImpl implements AwsCredentialsRepository {
    constructor(private readonly secretsManagerClient: SecretsManagerClient) {}
    ...
    async saveAwsCredentials(
        ...
        try {
            Logger.info(
                ...
                this.saveAwsCredentials.name,
                "executing create secret command",
            )
            const createSecretCommand = new CreateSecretCommand({
                Name: this.generateAwsCredentialsPath(
                    saveAwsCredentialsInput.ownerEmail,
                ),
            ...
            Logger.info(
                this.constructor.name,
                this.saveAwsCredentials.name,
                "create secret command executed with success",
            )
        } catch (error: any) {
            Logger.error(
                this.constructor.name,
                this.saveAwsCredentials.name,
                `Secrets manager client throw ${error.message}`,
            )
    ...
    async findAwsCredentials(
        ...
        try {
            Logger.info(
                this.constructor.name,
                this.findAwsCredentials.name,
                "executing get secret value command",
            )
            const getSecretValueCommand = new GetSecretValueCommand({
                SecretId: this.generateAwsCredentialsPath(
                    findAwsCredentialsInput.ownerEmail,
                ),
            })
            const output = await this.secretsManagerClient.send(
                getSecretValueCommand,
            )
            ...
            Logger.info(
                this.constructor.name,
                this.findAwsCredentials.name,
                "get secret value command executed with success",
            )
            const awsCredentials = JSON.parse(String(output?.SecretString))
            return new AwsCredentialsEntity(
    ...
    async updateAwsCredentials(
        ...
        try {
            Logger.info(
                this.constructor.name,
                this.updateAwsCredentials.name,
                "executing update secret command",
            )
            const updateSecretCommand = new UpdateSecretCommand({
                SecretId: this.generateAwsCredentialsPath(
                    updateAwsCredentialsInput.ownerEmail,
                ),
                SecretString: JSON.stringify({
            ...
            Logger.info(
                this.constructor.name,
                this.updateAwsCredentials.name,
                "update secret command executed with success",
            )
        } catch (error: any) {
            Logger.error(
                this.constructor.name,
    ...
    async deleteAwsCredentials(
        ...
        try {
            Logger.info(
                this.constructor.name,
                this.deleteAwsCredentials.name,
                "executing delete secret command",
            )
            const deleteSecretCommand = new DeleteSecretCommand({
                SecretId: this.generateAwsCredentialsPath(
                    deleteAwsCredentialsInput.ownerEmail,
                ),
            ...
            Logger.info(
                this.constructor.name,
                this.deleteAwsCredentials.name,
                "delete secret command executed with success",
            )
        } catch (error: any) {
            Logger.error(
                this.constructor.name,
