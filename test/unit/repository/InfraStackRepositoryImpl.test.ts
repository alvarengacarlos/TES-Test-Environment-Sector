...
describe("InfraStackRepositoryImpl", () => {
    ...
    const infraStackRepository = new InfraStackRepositoryImpl(getCloudFormationClientWithCredenialsMock)
    ...
    describe("createInfraStack", () => {
        ...
        test("should throw CloudFormationException", async () => {
            cloudFormationClientMocked.on(CreateStackCommand).rejects(new Error())

            await expect(infraStackRepository.createInfraStack(createInfraStackInput)).rejects.toThrow(CloudFormationException)
        })

        test("should create infra stack", async () => {
            cloudFormationClientMocked.on(CreateStackCommand).resolves({})

            await expect(infraStackRepository.createInfraStack(createInfraStackInput)).resolves.not.toThrow(CloudFormationException)
        })
    })
    ...
    describe("findInfraStacks", () => {
        ...
        test("should throw CloudFormationException", async () => {
            cloudFormationClientMocked.on(DescribeStacksCommand).rejects(new Error())

            await expect(infraStackRepository.findInfraStacks(findInfraStacksInput)).rejects.toThrow(CloudFormationException)
        })
        ...
        test("should find infra stacks", async () => {
            ...
            await expect(infraStackRepository.findInfraStacks(findInfraStacksInput)).resolves.not.toThrow(CloudFormationException)
        })
    })
    ...
    describe("deleteInfraStack", () => {
        ...
        test("should throw CloudFormationException", async () => {
            cloudFormationClientMocked.on(DeleteStackCommand).rejects(new Error())

            await expect(infraStackRepository.deleteInfraStack(deleteInfraStackInput)).rejects.toThrow(CloudFormationException)
        })

        test("should delete infra stack", async () => {
            cloudFormationClientMocked.on(DeleteStackCommand).resolves({})

            await expect(infraStackRepository.deleteInfraStack(deleteInfraStackInput)).resolves.not.toThrow(CloudFormationException)
        })
    })
})