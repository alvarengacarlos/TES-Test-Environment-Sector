...
describe("AwsCredentialsRepositoryImpl", () => {
    ...
    describe("saveAwsCredentials", () => {
        ...
        test("should throw SecretsManagerException", async () => {
            secretsManagerClientMocked.on(CreateSecretCommand).rejects(new Error())

            await expect(awsCredentialsRepository.saveAwsCredentials(saveAwsCredentialsInput)).rejects.toThrow(SecretsManagerException)
        })

        test("should save aws credentials", async () => {
            secretsManagerClientMocked.on(CreateSecretCommand).resolves({})

            await expect(awsCredentialsRepository.saveAwsCredentials(saveAwsCredentialsInput)).resolves.not.toThrow(SecretsManagerException)
        })
    })
    ...
    describe("findAwsCredentials", () => {
        ...
        test("should throw SecretsManagerException", async () => {
            ...
            await expect(awsCredentialsRepository.findAwsCredentials(findAwsCredentialsInput)).rejects.toThrow(SecretsManagerException)
        })
        ...
        test("should find aws credentials", async () => {
            ...
            await expect(awsCredentialsRepository.findAwsCredentials(findAwsCredentialsInput)).resolves.not.toThrow(SecretsManagerException)
        })
    })
    ...
    describe("updateAwsCredentials", () => {
        ...
        test("should throw SecretsManagerException", async () => {
            secretsManagerClientMocked.on(UpdateSecretCommand).rejects(new Error())

            await expect(awsCredentialsRepository.updateAwsCredentials(updateAwsCredentialsInput)).rejects.toThrow(SecretsManagerException)
        })

        test("should update aws credentials", async () => {
            secretsManagerClientMocked.on(UpdateSecretCommand).resolves({})

            await expect(awsCredentialsRepository.updateAwsCredentials(updateAwsCredentialsInput)).resolves.not.toThrow(SecretsManagerException)
        })
    })
    ...
    describe("deleteAwsCredentials", () => {
        ...
        test("should throw SecretsManagerException", async () => {
            secretsManagerClientMocked.on(DeleteSecretCommand).rejects(new Error())

            await expect(awsCredentialsRepository.deleteAwsCredentials(deleteAwsCredentialsInput)).rejects.toThrow(SecretsManagerException)
        })

        test("should delete aws credentials", async () => {
            secretsManagerClientMocked.on(DeleteSecretCommand).resolves({})

            await expect(awsCredentialsRepository.deleteAwsCredentials(deleteAwsCredentialsInput)).resolves.not.toThrow(SecretsManagerException)
        })
    })
})