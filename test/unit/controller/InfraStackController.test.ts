...
describe("InfraStackController", () => {
    ...
    describe("createInfraStack", () => {
        ...
        test("should return internal server error http response", async () => {
            jest.spyOn(infraStackService, "createInfraStack").mockRejectedValue(new Error())

            const httpResponse = await infraStackController.createInfraStack(httpRequest)

            expect(infraStackService.createInfraStack).toBeCalledWith(httpRequest.data)
        ...
        test("should return created http response", async () => {
            ...
            jest.spyOn(infraStackService, "createInfraStack").mockResolvedValue(createInfraStackDtoOutput)

            const httpResponse = await infraStackController.createInfraStack(httpRequest)

            expect(infraStackService.createInfraStack).toBeCalledWith(httpRequest.data)
            expect(httpResponse).toEqual(HttpResponse.created("Infra stack created with success", createInfraStackDtoOutput))
        })
    })
    ...
    describe("findInfraStacks", () => {
        ...
        test("should return internal server error http response", async () => {
            jest.spyOn(infraStackService, "findInfraStacks").mockRejectedValue(new Error())

            const httpResponse = await infraStackController.findInfraStacks(httpRequest)

            expect(infraStackService.findInfraStacks).toBeCalledWith(httpRequest.data)
            expect(httpResponse).toEqual(HttpResponse.internalServerError())
        })

        test("should return ok http response", async () => {
            const findInfraStacksDtoOutput = new FindInfraStacksDtoOutput([])
            jest.spyOn(infraStackService, "findInfraStacks").mockResolvedValue(findInfraStacksDtoOutput)

            const httpResponse = await infraStackController.findInfraStacks(httpRequest)

            expect(infraStackService.findInfraStacks).toBeCalledWith(httpRequest.data)
            expect(httpResponse).toEqual(HttpResponse.ok("Infra stacks got with success", findInfraStacksDtoOutput))
        })
    })
    ...
    describe("deleteInfraStacks", () => {
        ...
        test("should return internal server error http response", async () => {
            jest.spyOn(infraStackService, "deleteInfraStack").mockRejectedValue(new Error())

            const httpResponse = await infraStackController.deleteInfraStack(httpRequest)

            expect(infraStackService.deleteInfraStack).toBeCalledWith(httpRequest.data)
            expect(httpResponse).toEqual(HttpResponse.internalServerError())
        })

        test("should return ok http response", async () => {
            const deleteInfraStackDtoOutput = new DeleteInfraStackDtoOutput()
            jest.spyOn(infraStackService, "deleteInfraStack").mockResolvedValue(deleteInfraStackDtoOutput)

            const httpResponse = await infraStackController.deleteInfraStack(httpRequest)

            expect(infraStackService.deleteInfraStack).toBeCalledWith(httpRequest.data)
            expect(httpResponse).toEqual(HttpResponse.ok("Infra stack deleted with success", deleteInfraStackDtoOutput))
        })
    })
})