...
import {
    ...
    CreateInfraStackDtoOutput, DeleteInfraStackDtoInput, FindInfraStacksDtoInput, FindInfraStacksDtoOutput,
    InfraStackService
...
describe("InfraStackService", () => {
    ...
    describe("createInfraStack", () => {
        ...
        test("should create infra stack", async () => {
            ...
            const getContainerModelTemplateDtoOutput = new GetContainerModelTemplateDtoOutput(templateBody)
            jest.spyOn(templateService, "getContainerModelTemplate").mockResolvedValue(getContainerModelTemplateDtoOutput)
            jest.spyOn(infraStackRepository, "createInfraStack").mockResolvedValue()

            const output = await infraStackService.createInfraStack(createInfraStackDtoInput)
    ...
    describe("findInfraStacks", () => {
        ...
        test("should find infra stacks", async () => {
            ...
            jest.spyOn(infraStackRepository, "findInfraStacks").mockResolvedValue([infraStackEntity])

            const output = await infraStackService.findInfraStacks(findInfraStacksDtoInput)
            ...
            expect(output).toEqual(new FindInfraStacksDtoOutput([infraStackEntity]))
        })
    })
    ...
    describe("deleteInfraStack", () => {
        ...
        test("should delete infra stack", async () => {
            const findAwsCredentialsDtoOutput = new FindAwsCredentialsDtoOutput(accessKeyId, secretAccessKey)
            jest.spyOn(awsCredentialsService, "findAwsCredentials").mockResolvedValue(findAwsCredentialsDtoOutput)
            jest.spyOn(infraStackRepository, "deleteInfraStack").mockResolvedValue()

            const output = await infraStackService.deleteInfraStack(deleteInfraStackDtoInput)
