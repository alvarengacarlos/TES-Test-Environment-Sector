...
describe("SourceCodeService", () => {
    ...
    describe("uploadSourceCode", () => {
        ...
        test("should upload source code", async () => {
            ...
            const output = await sourceCodeService.uploadSourceCode(
                uploadSourceCodeDtoInput,
            )
    ...
    describe("findSourceCodes", () => {
        ...
        test("should find source codes", async () => {
            ...
            const sourceCodeEntity = new SourceCodeEntity(
                `${email}-${faker.animal.cat()}-sourceCode.zip`,
            )
            jest.spyOn(
                sourceCodeRepository,
                "findSourceCodes",
            ).mockResolvedValue([sourceCodeEntity])

            const output = await sourceCodeService.findSourceCodes(
                findSourceCodesDtoInput,
            )
            ...
            expect(output).toEqual(
                new FindSourceCodesDtoOutput([sourceCodeEntity]),
            )
        })
    })
    ...
    describe("deleteSourceCode", () => {
        ...
        test("should delete source code", async () => {
            ...
            const output = await sourceCodeService.deleteSourceCode(
                deleteSourceCodeDtoInput,
            )
